...
class Graph{
    ...
    public:
        ...
        //function to print min cut edges
        void min_cut_edges(ll s , ll t ,vector< vector<pair<ll,ll>> > &radj) ;
...
//function to print min cut edges
void Graph::min_cut_edges(ll s , ll t ,vector<vector<pair<ll,ll>>>&radj){
    ...
    while (!q.empty()){
        ...
        q.pop();
        ll vec_size=radj[u].size();
        for (ll v=0;v<vec_size;v++){

            if (!visited[radj[u][v].first] && radj[u][v].second > 0)
            {
                q.push(radj[u][v].first);
                visited[radj[u][v].first] = true;
            }
        }
    }

    //Printing  min  cut edges
    for(ll i=0;i<nodes;i++){
        ll vec_size=adj[i].size();

        for(ll j=0;j<vec_size;j++){

            if(visited[i] && !visited[adj[i][j].first]){
                cout<<"Edge :"<<" "<<i<<" -> "<<adj[i][j].first<<"\n";
            }
...
void Graph::ford_Fulkerson(ll s, ll t){
    ...
    min_cut_edges(s,t,rg.adj) ;

    cout<<"The min cut value for given flow network is : " << maxflow ;
