...
void Graph::ford_Fulkerson(ll s, ll t){
    ...
    cout<<"The maximum matchings for given graph is : " << maxflow ;
...
int main(){
    ...
    //output = freopen("moreno_out.txt", "w", stdout);

    ll  nodes_1,nodes_2; /* Number of nodes_1 and nodes_2 represent no of elements in respective bipartite sets*/
    ll  edges; /* Number of edges */
    ...
    //CREATING A ADJACENCY LIST FOR (nodes_1+nodes_2+2) VERTICES
    for(ll i = 0;i < edges;++i){
        ll x, y ;
        cin >> x >> y ;
        g.addEdge(x,nodes_1+y,1);
    }


    //adding edges from source to nodes_1
    for(ll i=1;i<=nodes_1;i++){
        g.adj[0].push_back({i,1}) ;
    }

    //adding edges from nodes_2 to sink
    for(ll i=nodes_1+1; i<=nodes_1+nodes_2; i++){
        g.adj[i].push_back({nodes_1+nodes_2+1,1}) ;
    }
